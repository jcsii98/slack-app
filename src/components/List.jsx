export default function List(props) {
  ...
  return (
    <div className="container-fluid d-flex flex-column justify-content-center p-0">
      ...
      <ul
        className="container-fluid d-flex flex-column gap-2 p-0"
        style={{ fontSize: '0.9rem', overflow: 'auto', maxHeight: '15rem' }}
      >
        {data.length === 0 && (
        ...
        {data.map((item) => {
          return (
            <li
              key={item.id}
              className="container-fluid d-flex justify-content-start align-items-center"
            >
              {title === 'Channels' ? (
                <i
