...
function ChatHeader(props) {
  ...
  const seeMembers = async () => {
    ...
    const ownerUser = allUsers.find((user) => user.id === response.data.data.owner_id);
  ...
  return (
    <>
      ...
      <Modal.Body className='container-fluid p-0 d-flex flex-column gap-4' style={{fontSize: "1.2rem", overflow: "hidden", minHeight: "32rem", maxHeight: "32rem"}}>
        <Tabs className='d-flex flex-row' defaultActiveKey={2} id="uncontrolled-tab-example" style={{padding: "1rem 1rem 0rem 1rem"}}>
          <Tab className='container-fluid rounded p-2' eventKey={1} title="About" >
          ...
          <Tab className='p-4 container-fluid d-flex flex-column gap-3' eventKey={2} title={`Members ${channelMembers.length}`}>
            <div className='container-fluid d-flex p-0 gap-2'>
              <input className='flex-grow-1 p-2' style={{height: "20%"}} type="text" onChange={handleChange}></input>
              <button className='btn btn-primary' style={{width: "20%"}} onClick={addMember}>Add</button>
            </div>
            <ul className='d-flex flex-column justify-content-center  align-items-start px-4 gap-3' style={{overflow: "auto", maxHeight: "20rem"}}>
      ...
      <Modal.Footer >
        <div style={{marginRight: "auto"}}>Channel ID: {channelData.id}</div>
      </Modal.Footer>
     </Modal>
     ...
     <div className="container-fluid bg-white d-flex align-items-center border">
      ...
      { receiverClass === "Channel" && <div className="btn p-2 border" style={{marginLeft: "auto",cursor: "pointer"}} onClick={seeMembers}>Members</div>}
    </div>
    </>
  );
}
